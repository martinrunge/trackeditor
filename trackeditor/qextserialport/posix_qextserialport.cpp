...
void Posix_QextSerialPort::init()
{
	...
	{
		qWarning("POSIX event driven mechanism still experimental");
	}
}
    ...
    if (!isOpen()) {
        ...
        if ((fd = ::open(port.toAscii() ,O_RDWR | O_NOCTTY | O_NDELAY)) != -1) {
            ...
            if (queryMode() == QextSerialBase::EventDriven)
            {
            	...
            	connect(notifier, SIGNAL(activated(int)), this, SLOT(notified(int)));
            }
        } else {
...
void Posix_QextSerialPort::close()
{
    ...
    if (queryMode() == QextSerialBase::EventDriven)
    {
    	disconnect(notifier, SIGNAL(activated(int)), this, SLOT(notified(int)));
    	delete notifier;
    	notifier = 0;
    }
...
void Posix_QextSerialPort::notified(int source_fd)
{
    if( isOpen() && source_fd == fd )
    {
    	emit readyRead();
    }
}
